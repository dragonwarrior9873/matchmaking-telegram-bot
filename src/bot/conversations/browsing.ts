...
tokenSelection += `${i + 1}. **${project.name.replace(/[*_`[\]()~>#+=|{}.!-]/g, '\\$&')}**\n`;
keyboard
...
if (isPhotoMessage) {
await response.editMessageCaption({
caption: `**Liked ${project.name.replace(/[*_`[\]()~>#+=|{}.!-]/g, '\\$&')}**\n\nIf they like you back, it's a match!`,
parse_mode: "Markdown",
});
} else {
await response.editMessageText(
`**Liked ${project.name.replace(/[*_`[\]()~>#+=|{}.!-]/g, '\\$&')}**\n\nIf they like you back, it's a match!`,
{
parse_mode: "Markdown",
}
);
...
if (isPhotoMessage) {
await response.editMessageCaption({
caption: ` **Passed on ${project.name.replace(/[*_`[\]()~>#+=|{}.!-]/g, '\\$&')}**`,
parse_mode: "Markdown",
});
} else {
await response.editMessageText(` **Passed on ${project.name.replace(/[*_`[\]()~>#+=|{}.!-]/g, '\\$&')}**`, {
parse_mode: "Markdown",
});
}
...
let card = `${currentIndex} of ${totalProjects}\n`;
card += `💖 Token : **${project.name.replace(/[*_`[\]()~>#+=|{}.!-]/g, '\\$&')}**\n\n`;
...
card += `💖 **Description:** ${project.description.replace(/[*_`[\]()~>#+=|{}.!-]/g, '\\$&')}\n`;
}
...
card += `• 💖 **Price:** $${project.token_price.toFixed(6)}${changeText}\n`;
}
...
socialLinks.push(`[X (Twitter)](https://x.com/${project.x_account.replace('@', '')})`);
}
